...
AuthService.$inject  = ['$auth','$state','userService','localStorageService'];
function AuthService($auth,$state,userService,localStorageService){
var avatar;
var Auth = {
  login: login,
  logout: logout,
...
function login(user,collback){
  ...
  .then(response => {
    ...
    userService.get({id:Auth.idUser()}).$promise
    .then(response =>{
